...
export default function TimesheetIngestion() {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Timesheet Ingestion</h1>
        ...
        <div className="flex flex-col items-center gap-4">
          <div
            className={`flex h-16 w-16 items-center justify-center rounded-2xl transition-all duration-300 ${
              isDragOver ? "gradient-accent scale-110" : "bg-secondary"
            }`}
          >
            <Upload className={`h-8 w-8 ${isDragOver ? "text-accent-foreground" : "text-muted-foreground"}`} />
          </div>
          <div>
      ...
      <div className="rounded-xl border border-border bg-card shadow-card">
        ...
        <div className="divide-y divide-border">
          {files.map((file) => {
            ...
            return (
              <div key={file.id} className="flex items-center justify-between p-4 animate-fade-in">
                ...
                <div className="flex items-center gap-4">
                  {file.confidence && (
                    <div className="text-right">
                      <p className="text-sm font-medium text-card-foreground">{file.confidence}%</p>
